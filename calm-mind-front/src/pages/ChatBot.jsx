...
export default function ChatBotStressBoard({ tasks = [] }) {
  ...
  const sendThroughBot = async (content) => {
    ...
    if (!user?.id) {
      pushMessage(
        "assistant",
        "User not logged in. Please refresh or login again."
      );
      return;
    }
    ...
    try {
      ...
      pushMessage(
        "assistant",
        response.data.response || "Sorry, no reply received."
      );
    } catch (error) {
      console.error(
  ...
  const [customTags, setCustomTags] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem(LS_CUSTOM_TAGS) || "[]");
    } catch {
      return [];
    }
  });
  ...
  const addLog = (extraNote) => {
    ...
    const tasksToday = tasks.filter(
      (t) => t?.dueDate && dayjs(t.dueDate).isSame(todayKey, "day")
    ).length;
    const log = {
      id: `log-${Date.now()}`,
      ts: now,
      ...
      tags,
      note: extraNote || note,
      tasksCount: tasksToday,
    };
    setLogs((prev) => [log, ...prev]);
  };

  const handleSaveAndAsk = async () => {
    const summary = `Stress: ${stress}/5; Tags: ${
  ...
  const dailySeries = last7.map((d) => ({
    ...
    value: avg(dailyMap.get(toDateKey(d.toISOString())) || []),
  }));

  const last8Weeks = Array.from({ length: 8 }, (_, i) =>
    today.subtract(7 * (7 - i), "day")
  );
  const weeklySeries = last8Weeks.map((d) => {
    const wk = weekStartKey(d);
    return {
      label: dayjs(wk).format("MM/DD"),
      value: avg(weeklyMap.get(wk) || []),
    };
  });

  const last12Months = Array.from({ length: 12 }, (_, i) =>
    today.subtract(11 - i, "month")
  );
  const monthlySeries = last12Months.map((d) => {
    const mk = monthKey(d);
    return { label: d.format("MMM"), value: avg(monthlyMap.get(mk) || []) };
  });

  const last5Years = Array.from({ length: 5 }, (_, i) =>
    today.subtract(4 - i, "year")
  );
  const yearlySeries = last5Years.map((d) => {
    const yk = d.format("YYYY");
    return { label: yk, value: avg(yearlyMap.get(yk) || []) };
  });
  ...
  const chartViews = [
    {
      key: "daily",
      label: "Daily Average (7d)",
      data: dailySeries,
      color: AMBER[500],
    },
    {
      key: "weekly",
      label: "Weekly Average (8w)",
      data: weeklySeries,
      color: AMBER[600],
    },
    {
      key: "monthly",
      label: "Monthly Trend (12m)",
      data: monthlySeries,
      color: AMBER[500],
    },
    {
      key: "yearly",
      label: "Yearly Trend (5y)",
      data: yearlySeries,
      color: AMBER[600],
    },
  ];
  const [chartView, setChartView] = useState(
    () => localStorage.getItem(LS_CHART_VIEW) || "daily"
  );
  useEffect(() => {
    localStorage.setItem(LS_CHART_VIEW, chartView);
  }, [chartView]);
  const activeView =
    chartViews.find((v) => v.key === chartView) || chartViews[0];
              ...
              >
                <div className="mb-4">
                  <div className="text-lg font-bold">
                    Log Stress & Message Coach
                  </div>
                  <div className="text-xs text-gray-500">
                    Sending will save a log and post a message to the chatbot
                    above.
                  </div>
                </div>
                      ...
                      return (
                        <button
                          ...
                          className={`px-3 py-1.5 rounded-full border text-sm ${
                            active ? "bg-[#222322] text-white" : "bg-white"
                          }`}
                          style={{
                            boxShadow: "0 4px 10px rgba(11,18,40,0.03)",
                          }}
                        >
                          {t}
                        </button>
                    ...
                    {!addingTag ? (
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          className="px-3 py-1.5 rounded-full bg-[#222322] text-white text-sm"
                        >
                          Add
                        </button>
                        <button
                          ...
                          className="px-3 py-1.5 rounded-full bg-white border text-sm"
                        >
                          Cancel
                        </button>
                      </div>
                ...
                <div className="mt-4 flex flex-col sm:flex-row gap-2">
                  <input
                    ...
                    onKeyDown={(e) =>
                      e.key === "Enter" && !e.shiftKey && handleSaveAndAsk()
                    }
                  ...
                  <button
                    ...
                    className="px-4 py-3 rounded-xl bg-[#222322] text-white"
            ...
            <aside className="col-span-12 lg:col-span-4 flex flex-col gap-3 overflow-y-auto pb-2">
              ...
              >
                ...
                <div className="text-sm text-gray-700">
                  You have{" "}
                  <span className="font-semibold">{dueToday.length}</span> task
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                      ...
                      <Line
                        ...
                        strokeWidth={3}
                        dot={{ r: 3, stroke: activeView.color }}
                      />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
                  ...
                  <div className="flex items-center gap-2">
                    {!editMode ? (
                      <button
                        ...
                        className="px-3 py-1.5 rounded-full bg-white border text-sm transition-all hover:bg-gray-50"
                      >
                        Edit
                      </button>
                    ) : (
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          className="px-3 py-1.5 rounded-full bg-[#222322] text-white text-sm transition-all"
                        ...
                        >
                          <div className="flex gap-3">
                            {editMode && (
                              <input
                                type="checkbox"
                                className="mt-1 h-4 w-4 accent-amber-500 transition"
                                checked={checked}
