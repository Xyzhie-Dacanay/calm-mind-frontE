    ...
    <div className="overflow-x-auto bg-white border rounded-xl shadow-sm">
      <table className="min-w-full text-sm">
        <thead className="text-left bg-gray-50">
          <tr>
            {["Title", "Priority", "Start", "Due", "Status", "Actions"].map((header) => (
        ...
        <tbody>
          {rows.length > 0 ? (
            rows.map((t, index) => (
              <tr key={`${t._id}-${index}`} className="border-t hover:bg-gray-50">
                <td className="px-4 py-3">{t.title}</td>
                <td className="px-4 py-3">{t.priority}</td>
                <td className="px-4 py-3">{t.startDate}</td>
                <td className="px-4 py-3">{t.dueDate}</td>
                <td className="px-4 py-3 capitalize">{statusLabel[t.eff] || t.eff}</td>
                      ...
                      <React.Fragment key={`todo-actions-${t._id}-${index}`}>
                        ...
                        <label className="inline-flex items-center gap-2 text-xs">
                          <input
                            type="checkbox"
                            className="h-4 w-4"
                            onChange={(e) => e.target.checked && onComplete(t._id)}
                          />
                          Completed
                        </label>
                      </React.Fragment>
